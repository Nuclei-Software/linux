...
static void __xec_eth_reset(struct netdata_local *pldat)
{
	...
	/* Reset all MAC logic */
	val = readl(pldat->misc_base + 0x20);
	val &= ~BIT(14);
	writel(val, pldat->misc_base + 0x20);
	val |= BIT(14);
	writel(val, pldat->misc_base + 0x20);
	
	pldat->duplex = DUPLEX_FULL;
	pldat->speed = SPEED_10;
}

static int __xec_mii_mngt_reset(struct netdata_local *pldat)
{
	if (pldat->ndev->phydev)
		return genphy_soft_reset(pldat->ndev->phydev);
	return 0;
}
...
static int xec_mii_probe(struct net_device *ndev)
{
	...
	xec_mdio_write(pldat->mii_bus, phydev->phy_id, MII_BMCR, BMCR_RESET);
...
static int xec_eth_drv_probe(struct platform_device *pdev)
{
	...
	pldat->misc_base = ioremap(SOC_MISC_BASE, 4096);
	...
	ret = of_property_read_u64_array(np, "desc_mem",  &pldat->dma_buff_base_p, 1);
	if (!ret) {
		pldat->dma_buff_base_v = ioremap(pldat->dma_buff_base_p, pldat->dma_buff_size);
