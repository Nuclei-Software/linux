...
void dma_tx_start( struct net_device *dev ,int ip_summed,unsigned short pkt_len, unsigned char *data)
{
	...
	memcpy(mac_tx_buf+8, data, pkt_len);//put data into tx buffer
		...
		int i=0;
		printk("mac_tx_buf:\n");
		for(i=0;i<pkt_align_len+8;i++)
		{
			printk("%d:0x%2x ",i-8,*(mac_tx_buf+i));
		}
		printk("\n");
#endif
...
//printk("#priv->stats.rx_packets:0x%x,priv->stats.rx_bytes:0x%x\n",priv->stats.rx_packets,priv->stats.rx_bytes);
//printk("#priv->stats.rx_errors:0x%x,priv->stats.rx_missed_errors:0x%x\n",priv->stats.rx_errors,priv->stats.rx_missed_errors);
...
/*	ndev = alloc_etherdev(sizeof(struct mac_eth_priv));
	if (!ndev)
		return -1;	
*/
	ndev = alloc_netdev(sizeof(struct mac_eth_priv), "vir%d", 0,mac_eth_init);
	if(!ndev) 
	{
